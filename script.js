...
button.addEventListener('click', function () {
    ...
    console.log(`単価は${単価}`);  // ここで一人当たりの単価が正しく取得できていることを確認する。

    let 全体のチケット代 = 全体のチケット代を計算する(単価, 人数);
    console.log(`全体のチケット代は${全体のチケット代}`);  //ここで全体のチケット代が正しく計算できていることを確認する。

    let クーポンを考慮した全体のチケット代 = クーポンを適用する(全体のチケット代, クーポンの有無);
    console.log(`クーポンを考慮した全体のチケット代は${クーポンを考慮した全体のチケット代}`);    // ここでクーポンを考慮した全体のチケット代が正しく計算できているかを確認する。

    let ラッキー7を考慮した全体のチケット代 = ラッキー7を適用する(日付, クーポンを考慮した全体のチケット代);
    console.log(`ラッキー7を考慮した全体のチケット代は${ラッキー7を考慮した全体のチケット代}`);   // ここでラッキー7を考慮した全体のチケット代が正しく計算できているかを確認する。
